...
describe('operators', () => {
  ...
  describe('om', () => {
    it('should correctly map to given object schema', () => {
      const mockValue = 'TEST';
      const test = take<TestFromObject, 'test.nested.value'>(
        'test.nested.value'
      );
      const mapObject = om({
        test
    ...
    it('should be able to nestle om in same schema', () => {
      ...
      const takeNestedValue = take<TestFromObject, 'test.nested.value'>(
        'test.nested.value'
      );

      const takeBoolValue = take<TestFromObject, 'test.nested2.boolValue'>(
        'test.nested2.boolValue'
      );
